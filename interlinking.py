...
# --- New: Find ALL internal URLs (Hybrid: Sitemap + Fallback Crawling) ---
            ...
            if response.status_code == 200 and "<urlset" in response.text:
                ...
                for url in root.findall(".//{*}loc"):
                    ...
                    if home_url in loc:
                        all_urls.add(loc)
                if all_urls:
                    return sorted(all_urls)  # ✅ Return URLs from sitemap directly
        except:
            continue

    # --- Step 2: Fallback to HTML crawling ---
    visited = set()
    queue = [home_url]
